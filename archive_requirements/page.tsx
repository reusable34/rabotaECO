  ...
  const loadData = async () => {
    try {
      ...
      reqs = reqs.filter(req => {
        ...
        if (seenTitles.has(req.title)) return false;
        seenIds.add(req.id);
      ...
      const conts = Array.isArray(contractsRes.data) ? contractsRes.data : (contractsRes.data?.items || []);
  ...
  const handleRecalculate = async () => {
    ...
    try {
      ...
      const requestData: any = {
        ...
        has_well: hasWell === '' ? (client.has_well || false) : (hasWell === true || hasWell === 'true' || hasWell === 1 || hasWell === '1'),
        has_river: hasRiver === '' ? (client.has_river || false) : (hasRiver === true || hasRiver === 'true' || hasRiver === 1 || hasRiver === '1'),
        has_byproduct: hasByproduct === '' ? (client.has_byproduct || false) : (hasByproduct === true || hasByproduct === 'true' || hasByproduct === 1 || hasByproduct === '1'),
      ...
      if (response.data.success) {
        ...
        try {
          ...
          console.log('Первые 3 требования:', reqs.slice(0, 3).map(r => r.title));
